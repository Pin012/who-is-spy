  ...
  return (
    <div className="glass p-10 rounded-2xl shadow-2xl max-w-2xl mx-auto space-y-8 animate-in zoom-in duration-300 border border-white/5">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-black text-white italic tracking-tighter uppercase">Waiting for Squad</h2>
          <p className="text-gray-500 text-xs font-bold tracking-widest uppercase mt-1">Ready: {players.length} / 12</p>
        </div>
        <div onClick={copyCode} className="cursor-pointer bg-red-600/10 border border-red-500/20 px-6 py-3 rounded-lg text-center group transition-all hover:bg-red-600/20">
          <p className="text-[10px] text-red-500 font-black uppercase tracking-widest">Access Code</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input 
                ...
                placeholder="例如：珍珠奶茶"
                className="w-full bg-black/40 border border-white/5 rounded-lg px-4 py-3 text-sm focus:ring-1 focus:ring-red-500 outline-none text-white font-bold"
              />
            </div>
            <div className="space-y-1">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {players.map((p) => (
          <div key={p.id} className="bg-white/5 border border-white/10 p-4 rounded-xl text-center flex flex-col items-center gap-2 transition-all hover:border-white/20">
        ...
        {Array.from({ length: Math.max(0, 6 - players.length) }).map((_, i) => (
          <div key={`empty-${i}`} className="border border-dashed border-white/5 rounded-xl flex items-center justify-center h-28 opacity-20">
      ...
      {currentPlayer.is_host ? (
        <button
          ...
          className="w-full bg-red-600 hover:bg-red-500 text-white font-black py-5 rounded-xl shadow-2xl shadow-red-900/40 transition-all active:scale-[0.98] disabled:opacity-30 disabled:cursor-not-allowed uppercase tracking-widest text-lg"
        >
          {starting ? "Initializing Squad..." : (players.length < 4 ? `Need ${4 - players.length} more Agents` : "Deploy Mission")}
        </button>
      ) : (
        <div className="text-center bg-white/5 p-4 rounded-lg border border-white/5">
