...
const LobbyView: React.FC<LobbyViewProps> = ({ game, players, currentPlayer }) => {
  ...
  return (
    <div className="glass p-6 md:p-10 rounded-2xl shadow-2xl max-w-2xl mx-auto space-y-6 md:space-y-8 animate-in zoom-in duration-300 border border-white/5">
      <div className="flex flex-col md:flex-row justify-between items-start gap-4">
        <div className="space-y-1">
          <h2 className="text-2xl md:text-3xl font-black text-white italic tracking-tighter uppercase leading-tight">Mission Briefing</h2>
        ...
        <div onClick={copyCode} className="w-full md:w-auto cursor-pointer bg-red-600/10 border border-red-500/20 px-5 py-3 rounded-lg text-center group transition-all hover:bg-red-600/20">
      ...
      {!game.host_is_player && currentPlayer.is_host && (
        <div className="bg-white/5 border border-white/10 p-5 md:p-6 rounded-xl space-y-4 shadow-inner">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4">
        {host && (
          <div className={`p-4 rounded-xl border-2 text-center flex flex-col items-center gap-2 transition-all shadow-lg ${!game.host_is_player ? 'bg-amber-500/5 border-amber-500/20' : 'bg-white/5 border-white/10'}`}>
            <div className={`w-12 h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center text-lg md:text-xl font-black shadow-inner ${!game.host_is_player ? 'bg-amber-600 text-white' : 'bg-red-600 text-white'}`}>
              {host.name.charAt(0).toUpperCase()}
            </div>
            <span className="text-xs md:text-sm font-black truncate w-full text-gray-100">{host.name}</span>
            <span className={`text-[7px] md:text-[8px] px-2 py-0.5 rounded-full font-black tracking-widest border ${!game.host_is_player ? 'bg-amber-600/20 text-amber-500 border-amber-500/30' : 'bg-red-600/20 text-red-500 border-red-500/30'}`}>
