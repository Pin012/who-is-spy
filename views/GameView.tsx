...
const GameView: React.FC<GameViewProps> = ({ game, players, currentPlayer }) => {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-1000 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-black/80 p-6 rounded-xl border border-white/10 shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 left-0 w-1 h-full bg-red-600"></div>
        <div className="flex items-center gap-8">
          <div className={`px-5 py-2 rounded-md font-black uppercase tracking-[0.2em] text-[10px] shadow-2xl transition-all ${game.status === GameStatus.PLAYING ? 'bg-amber-500 text-black' : 'bg-red-600 text-white animate-pulse'}`}>
        ...
        {currentPlayer.is_host && (
          <button 
            onClick={togglePhase}
            className="bg-red-600 hover:bg-red-500 text-white px-10 py-4 rounded-lg font-black uppercase tracking-[0.2em] transition-all hover:scale-105 active:scale-95 shadow-xl shadow-red-900/40 text-sm"
          >
      ...
      <div className="grid lg:grid-cols-12 gap-10">
          ...
          {players.filter(p => game.host_is_player || !p.is_host).map((p) => {
            ...
            return (
              <div 
                ...
                className={`group relative overflow-hidden p-8 rounded-xl border-2 transition-all duration-300 flex flex-col items-center gap-4
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="glass p-10 rounded-2xl border border-white/5 text-center space-y-8 bg-black/60 shadow-2xl">
            <h3 className="text-[9px] font-black text-gray-700 uppercase tracking-[0.6em]">Access Card [Secure]</h3>
            
            <div 
              onClick={() => setRevealed(!revealed)}
              className={`aspect-[2/3] w-full max-w-[280px] mx-auto rounded-xl cursor-pointer transition-all duration-1000 relative preserve-3d shadow-2xl ${revealed ? '[transform:rotateY(180deg)]' : ''}`}
            >
              {/* Card Back */}
              <div className="absolute inset-0 bg-[#080808] rounded-xl flex flex-col items-center justify-center border-4 border-red-900/20 backface-hidden overflow-hidden">
              ...
              {/* Card Front */}
              <div className={`absolute inset-0 bg-black rounded-xl [transform:rotateY(180deg)] backface-hidden flex flex-col items-center p-10 space-y-5 shadow-2xl overflow-hidden border-[3px] ${isSpectator ? 'border-amber-600' : (currentPlayer.role === PlayerRole.UNDERCOVER ? 'border-red-600' : 'border-cyan-600')}`}>
                ...
                <div className="w-full space-y-3 py-6 relative">
                   ...
                   <div className="bg-gradient-to-br from-black to-zinc-950 border border-white/5 rounded-xl py-8 flex items-center justify-center shadow-inner border-red-900/20">
          ...
          <div className="bg-black/80 border border-white/5 p-8 rounded-xl shadow-2xl relative overflow-hidden">
            ...
            <p className="text-xs text-gray-500 leading-relaxed font-medium italic">
              {isSpectator 
                ...
                : (currentPlayer.role === PlayerRole.CIVILIAN 
                  ? "警告：敵方特務已滲透。仔細聆聽描述，找出細微的語意偏差。" 
                  : "滲透成功：你的詞彙與他人不同。保持冷靜，透過描述推斷平民詞，並建立掩護。")}
            </p>
          </div>
        </div>
      </div>
