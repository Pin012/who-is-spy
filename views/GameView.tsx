...
const GameView: React.FC<GameViewProps> = ({ game, players, currentPlayer, onExit }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-1000 max-w-7xl mx-auto px-4 pb-20 relative">
      ...
      <div className="fixed top-4 right-4 z-[200] flex items-center justify-end gap-2.5 max-w-[95vw]">
        {currentPlayer.is_host && (
          <>
            <button
        ...
        <div className="flex flex-col gap-1 w-full md:w-auto">
          <div className="flex items-center gap-6">
            <div className={`px-4 py-2 rounded-md font-black uppercase tracking-[0.1em] text-[13px] shadow-lg transition-all 
              ...
              {game.status === GameStatus.PLAYING ? `回合 ${game.round}  -  情報描述` : 
              ...
              {!canIInput ? (
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <input
                    ...
                    placeholder={game.status === GameStatus.DEFENDING ? "請發表最後的申冤描述..." : "請輸入你的描述...(不超過20字)"}
                  ...
                  <div className="text-left md:text-center w-full space-y-2 md:space-y-3">
                    ...
                    <div className="min-h-[52px] md:h-24 flex items-center justify-start md:justify-center w-full relative">
                      {p.is_alive ? (
                        ...
                        <div className={`absolute inset-0 flex items-center justify-center transition-all duration-700 ease-out ${isJustEliminated?'scale-[2.5] opacity-100':'scale-100 opacity-95'}`}><div className={`px-3 py-1 rounded text-[10px] font-black uppercase tracking-widest border ${p.role===PlayerRole.UNDERCOVER?'bg-red-600/35 text-red-300 border-red-400/50 shadow-[0_0_18px_rgba(220,38,38,0.35)]':'bg-cyan-500/30 text-cyan-200 border-cyan-300/50 shadow-[0_0_18px_rgba(34,211,238,0.25)]'}`}>{p.role===PlayerRole.UNDERCOVER?'臥底':'平民'}</div></div>
                      )}
                    </div>
                  </div>
                  ...
                  {!p.is_alive && (
                    <div className="absolute inset-0 z-30 pointer-events-none">
                      <div className={`absolute left-1/2 -translate-x-1/2 transition-all duration-700 ease-out ${isJustEliminated ? 'top-24 scale-[1.7] md:scale-[2] opacity-100' : 'top-[47px] scale-100 opacity-95'}`}>
                        <div className="bg-red-800/90 text-white/95 px-2.5 md:px-4 py-1 md:py-1.5 text-[10px] md:text-[16px] font-black uppercase tracking-[0.2em] md:tracking-[0.25em]
                          rotate-[-12deg]
            ...
            <div className="lg:hidden space-y-4">
              <div className="w-full max-w-sm mx-auto">
                <button
                  ...
                  className={`w-full inline-flex items-center justify-center gap-3 px-5 py-3 rounded-lg border-2 transition-all duration-300 font-black text-lg tracking-[0.08em] shadow-[0_8px_24px_rgba(0,0,0,0.35)] ${revealed ? 'bg-zinc-700 border-zinc-500 text-white hover:bg-zinc-600' : 'bg-amber-500 border-amber-300 text-black hover:bg-amber-400 animate-[pulse_1.6s_ease-in-out_infinite]'}`}
                >
