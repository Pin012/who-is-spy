...
export const generateWordPair = async () => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: "請產生一組全新、具創意、適合『誰是臥底』遊戲的詞語對。這兩個詞必須非常相似但不同，可使用職場或生活情境詞彙，也可使用工程顧問公司內部詞彙，題目請盡量多變化但不超過四字。直接回傳 JSON 物件，不要包含 markdown 標籤，包含 civilianWord (平民詞) 和 undercoverWord (臥底詞)。",
      config: {
